...
video_list = os.listdir(cwd+"/videos/converted/")
print("--- Will process "+str(len(video_list[5:10]))+" videos")

if write_video:
    out = cv2.VideoWriter(cwd+"/output/videos/processed.avi", fourcc, FPS, (W, H))

for input_video in video_list[5:10]:
    print(input_video)
...
for input_video in video_list[5:10]:
                ...
                if yolo_area > (FRAME_AREA*frame_fraction):
                    ...
                    if alpr_results["results"]:
                        ...
                        if re.match(r"^[A-Z]{3}[0-9]{3}$", plate):
                            ...
                            cv2.putText(frame, "placa: "+plate, (15, 100),
                                        font, 1.0, (0, 0, 255), 1)
                        else:
                            plate = np.nan
    ...
    print("--- Processing took: "+str(tic-time.time()))
